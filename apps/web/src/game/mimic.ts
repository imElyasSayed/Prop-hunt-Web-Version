...
export const SUSPECTS: Suspect[] = [
  { idx: 0, name: "You", color: "#ff2e9a" },
  { idx: 1, name: "Pip", color: "#0fd4e6" },
  { idx: 2, name: "Bloop", color: "#b4f531" },
  { idx: 3, name: "Gus", color: "#ff7a17" },
  { idx: 4, name: "Nova", color: "#8a4cff" },
];
 ...
 * line per non-mimic keeps it ambiguous.
 */
export function buildEvidence(): string[] {
  const lines: string[] = [];
  for (const s of mimic.sabotage) {
    const target = SUSPECTS[s.targetIdx]?.name ?? "an ally";
    lines.push(`0:${Math.floor(s.t).toString().padStart(2, "0")}  Someone ${KIND_LABEL[s.kind]} ${target}.`);
  }
  ...
  for (let i = 0; i < 3; i++) lines.push(`•  ${noise[i % noise.length]}`);
  if (!mimic.sabotage.length) {
    lines.push("⚠︎  No measurable sabotage detected this round.");
  }
  return lines;
}
